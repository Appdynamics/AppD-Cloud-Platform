...
1.	Install [AWS CLI 2.13.16](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2-mac.html).  
...
2.	Validate installed command-line tools:
    ...
    ```bash
    $ aws --version
    aws-cli/2.13.16 Python/3.11.5 Darwin/22.6.0 source/arm64 prompt/off
    ```
...
1.	Install [AWS CLI 2.13.16](https://awscli.amazonaws.com/AWSCLIV2.msi) for Windows 64-bit.  
...
2.	Validate installed command-line tool:
    ...
    ```bash
    $ aws --version
    aws-cli/2.13.16 Python/3.11.4 Windows/10 exe/AMD64 prompt/off
    ```
